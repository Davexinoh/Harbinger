...
export function registerCommands(bot) {
  ...
  bot.onText(/\/connect/, async (msg) => {
    const chatId = msg.chat.id;
    await upsertUser(chatId, msg.from?.username);
    await updateUser(chatId, { setup_step: SETUP_STEPS.AWAITING_PUB_KEY });
  ...
  bot.onText(/\/setup/, async (msg) => {
    ...
    await updateUser(chatId, { setup_step: SETUP_STEPS.AWAITING_THRESHOLD });
  ...
  bot.onText(/\/category/, async (msg) => {
    ...
    await updateUser(chatId, { setup_step: SETUP_STEPS.AWAITING_CATEGORY });
  ...
  bot.onText(/\/signals/, async (msg) => {
    ...
    try {
      ...
      const { crypto, sports, sentiment, btc15m, fx, composite } = signals;
      const status = composite >= 0.65 ? "🔥 THRESHOLD BREACH" : composite >= 0.55 ? "⚡ WARMING UP" : "📡 MONITORING";

      return bot.sendMessage(
        chatId,
        `📡 *Live Signal Report*\n\n` +
        `₿ *BTC 15m* ${btc15m?.direction === "UP" ? "🟢" : btc15m?.direction === "DOWN" ? "🔴" : "⚪"}\n` +
        `${signalBar(btc15m?.score || 0)}\n` +
        `${btc15m?.currentPrice ? `$${btc15m.currentPrice.toLocaleString()} | RSI: ${btc15m.rsi} | 15m: ${btc15m.change15m}%` : btc15m?.error || "No data"}\n\n` +
        ...
        `${sports.best ? sports.best.title?.slice(0, 55) || "Active market" : sports.reason || "No sports markets"}\n\n` +
        `*FX* ${fx?.direction === "bullish" ? "🟢" : fx?.direction === "bearish" ? "🔴" : "⚪"}\n` +
        `${signalBar(fx?.score || 0)}\n` +
        `${fx?.best ? `${fx.best.label} ${fx.best.change}%` : fx?.reason || "Warming up"}\n\n` +
        `*Sentiment* ${sentiment.direction === "bullish" ? "🟢" : sentiment.direction === "bearish" ? "🔴" : "⚪"}\n` +
        `${signalBar(sentiment.score)}\n` +
        `${sentiment.best ? `"${sentiment.best.title.slice(0, 55)}..."` : sentiment.reason || "No headlines"}\n\n` +
        `━━━━━━━━━━━━━━\n` +
        `*Composite* ⚡\n` +
        `${signalBar(composite)}\n\n` +
        `${status}\n\n` +
  ...
  bot.onText(/\/trades/, async (msg) => {
    const chatId = msg.chat.id;
    const trades = await getRecentTrades(chatId, 10);
  ...
  bot.onText(/\/pnl/, async (msg) => {
    const chatId = msg.chat.id;
    const stats  = await getPnL(chatId);
  ...
  bot.onText(/\/limit/, async (msg) => {
    const chatId = msg.chat.id;
    const user   = await getUser(chatId);
    await updateUser(chatId, { setup_step: SETUP_STEPS.AWAITING_LIMIT_UPDATE });
    return bot.sendMessage(
      chatId,
  ...
  bot.onText(/\/threshold/, async (msg) => {
    const chatId = msg.chat.id;
    const user   = await getUser(chatId);
    await updateUser(chatId, { setup_step: SETUP_STEPS.AWAITING_THRESHOLD_UPDATE });
    return bot.sendMessage(
      chatId,
  ...
  bot.onText(/\/disconnect/, async (msg) => {
    const chatId = msg.chat.id;
    await updateUser(chatId, { bayse_pub_key: null, bayse_sec_key: null, engine_active: 0, setup_step: null });
    return bot.sendMessage(
      chatId,
  ...
  bot.on("message", async (msg) => {
    ...
    switch (user.setup_step) {
      ...
      case SETUP_STEPS.AWAITING_PUB_KEY: {
        ...
        await updateUser(chatId, { bayse_pub_key: encrypt(text), setup_step: SETUP_STEPS.AWAITING_SEC_KEY });
        return bot.sendMessage(
          chatId,
      ...
      case SETUP_STEPS.AWAITING_THRESHOLD: {
        if (text.toLowerCase() === "default") {
          await updateUser(chatId, { threshold: 0.60, setup_step: SETUP_STEPS.AWAITING_MAX_AMOUNT });
        ...
        await updateUser(chatId, { threshold: val, setup_step: SETUP_STEPS.AWAITING_MAX_AMOUNT });
      ...
      case SETUP_STEPS.AWAITING_MAX_AMOUNT: {
        ...
        await updateUser(chatId, { max_trade_usd: val, setup_step: SETUP_STEPS.AWAITING_CURRENCY });
      ...
      case SETUP_STEPS.AWAITING_CURRENCY: {
        ...
        await updateUser(chatId, { currency: val, setup_step: null });
        return bot.sendMessage(
          chatId,
      ...
      case SETUP_STEPS.AWAITING_LIMIT_UPDATE: {
        ...
        await updateUser(chatId, { max_trade_usd: val, setup_step: null });
      ...
      case SETUP_STEPS.AWAITING_THRESHOLD_UPDATE: {
        ...
        await updateUser(chatId, { threshold: val, setup_step: null });
      ...
      case SETUP_STEPS.AWAITING_CATEGORY: {
        ...
        await updateUser(chatId, { preferred_category: val === "all" ? null : val, setup_step: null });
        return bot.sendMessage(
          chatId,
  ...
  bot.on("new_chat_members", async (msg) => {
    ...
    if (isBot) {
      await upsertGroup(msg.chat.id, msg.chat.title, msg.from?.id);
      bot.sendMessage(
        msg.chat.id,
  ...
  bot.on("left_chat_member", async (msg) => {
    if (msg.left_chat_member?.is_bot) await await removeGroup(msg.chat.id);
  });
}
...
export function registerTradeCommands(bot) {
  ...
  bot.onText(/\/trade/, async (msg) => {
    ...
    await updateUser(chatId, { setup_step: TRADE_STEPS.AWAITING_MARKET_SEARCH });
    return bot.sendMessage(
      chatId,
    ...
    switch (user.setup_step) {
      ...
      case TRADE_STEPS.AWAITING_MARKET_SEARCH: {
        ...
        try {
          ...
          await updateUser(chatId, { setup_step: TRADE_STEPS.AWAITING_OUTCOME });
      ...
      case TRADE_STEPS.AWAITING_OUTCOME: {
        ...
        if (pending?.event && pending?.openMarkets) {
          ...
          await updateUser(chatId, { setup_step: TRADE_STEPS.AWAITING_AMOUNT });
      ...
      case TRADE_STEPS.AWAITING_AMOUNT: {
        ...
        try {
          ...
          await updateUser(chatId, { setup_step: TRADE_STEPS.AWAITING_CONFIRM });
