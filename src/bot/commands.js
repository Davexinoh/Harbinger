  ...
  bot.on("message", async (msg) => {
    ...
    switch (user.setup_step) {
      ...
      case TRADE_STEPS.AWAITING_MARKET_SEARCH: {
        ...
        try {
          const { publicRequest } = await import("../bayse/client.js");
          const data = await publicRequest(`/v1/pm/events?status=open&limit=50`);
          const events = data?.data || [];
      ...
      case TRADE_STEPS.AWAITING_AMOUNT: {
        ...
        try {
          const { getQuote } = await import("../bayse/client.js");
          ...
          return bot.sendMessage(
            ...
            `Expected shares: \`${quote.expectedShares?.toFixed(2) || "—"}\`\n` +
            `Fee: \`${quote.fee?.toFixed(2) || "0"}\`\n` +
            `Total cost: \`${currency} ${quote.total?.toFixed(2) || amount}\`\n\n` +
      ...
      case TRADE_STEPS.AWAITING_CONFIRM: {
        ...
        try {
          const { placeOrder }  = await import("../bayse/client.js");
          const { decrypt }     = await import("../utils/encryption.js");
          ...
          insertTrade({
            ...
            amount:        pending.amount,
            currency,
            expected_price: pending.quote?.expectedPrice || null,
            status:        "open",
          });
          ...
          return bot.sendMessage(
            ...
            `Entry: \`${(pending.quote?.expectedPrice * 100).toFixed(1)}¢\`\n\n` +
