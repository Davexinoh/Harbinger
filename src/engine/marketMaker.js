...
import { placeLimitOrder, cancelOrder, getOpenOrders, getEvents } from "../bayse/client.js";
import { resolveOutcomeId } from "../bayse/client.js";
...
export async function startMarketMaking(user, eventId, marketId, options = {}) {
  ...
  const currency = user.currency || "NGN"; // MM works best in NGN for Bayse
...
async function runMakerCycle(user, pubKey, secKey, eventId, marketId, currency, spread, amount) {
  ...
  try {
    ...
    const bidPrice = Math.max(midpoint - spread, 0.05); // buy YES at midpoint - spread
    const askPrice = Math.min(midpoint + spread, 0.95); // sell YES at midpoint + spread
    ...
    const bidOrder = await placeLimitOrder(pubKey, secKey, eventId, marketId, {
      side:      "BUY",
      outcomeId: yesId,
      amount,
      price:     bidPrice,
    ...
    const askOrder = await placeLimitOrder(pubKey, secKey, eventId, marketId, {
      side:      "BUY",
      outcomeId: noId,
      amount,
      price:     1 - askPrice, // NO price = 1 - YES price
      currency,
    });
