...
export async function postCrowdPoll(signals, match) {
  ...
  for (const group of groups) {
    const existing = await getActivePoll(group.group_id);
...
export function registerPollHandler(botInstance) {
  ...
  botInstance.on("poll", async (poll) => {
    const stored = await getPollByTelegramId(String(poll.id));
    if (!stored) return;
    ...
    await updatePollVotes(String(poll.id), yesVotes, noVotes, unsureVotes);
    ...
    if (poll.is_closed) {
      ...
      if (stored.engine_direction) {
        const correct = await resolvePoll(String(poll.id), stored.engine_direction);
        console.log(
...
export async function getCrowdScore() {
  ...
  const scores = [];
  for (const group of groups) {
    const poll = await getActivePoll(group.group_id);
...
export async function getCrowdIQReport() { return getCrowdCalibration(); }
