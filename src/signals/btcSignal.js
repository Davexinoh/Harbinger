...
export async function runBTCSignal() {
  ...
  try {
    ...
    const momentumScore = 1 / (1 + Math.exp(-momentumRaw * 1.5));

    let rsiScore = 0.5;
    if (rsi > 55 && rsi < 75)  rsiScore = 0.7 + (rsi - 55) / 100;
    if (rsi < 45 && rsi > 25)  rsiScore = 0.3 - (45 - rsi) / 100;
    if (rsi >= 75)              rsiScore = 0.35;
    if (rsi <= 25)              rsiScore = 0.65;

    const volumeScore      = Math.min(volRatio / 3, 1) * 0.2 + 0.8;
    ...
    const direction  = (momentumUp && rsiUp) || (momentumUp && rsi <= 25) ? "UP" : "DOWN";
