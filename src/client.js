...
export async function placeLimitOrder(publicKey, secretKey, eventId, marketId, orderBody) {
  return signedRequest(publicKey, secretKey, "POST", `/v1/pm/events/${eventId}/markets/${marketId}/orders`, { ...orderBody, type: "LIMIT" });
}
...
export function resolveOutcomeId(market, suggestedOutcome) {
  ...
  // Shape 1: outcomes[] array (some Bayse markets use this)
  if (Array.isArray(market.outcomes) && market.outcomes.length >= 2) {
    // Map YES/UP/bullish → first outcome, NO/DOWN/bearish → second
  ...
  // No label match — use position convention: YES/UP→outcome1, NO/DOWN→outcome2
  if (isBullishTarget) {
