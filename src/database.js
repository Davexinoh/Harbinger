...
export function updatePollVotes(telegramPollId, votesYes, votesNo, votesUnsure) {
  ...
  // Crowd score: fraction voting YES, dampened toward 0.5 when unsure votes are high
...
export function resolvePoll(telegramPollId, actualOutcome) {
  ...
  const totalVotes = poll.votes_yes + poll.votes_no + poll.votes_unsure;
  ...
  const db = getDb();
  const existing = db.prepare("SELECT * FROM crowd_calibration WHERE category = 'general'").get();
  if (existing) {
    db.prepare(`
    ...
    db.prepare(`
      INSERT INTO crowd_calibration (category, total_polls, correct)
      VALUES ('general', 1, ?)
    `).run(crowdWasRight);
  }
